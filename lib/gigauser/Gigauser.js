...
class Gigauser extends ReadyResource {
  ...
  async addRoom(roomData) {
    ...
    const existingIndex = this._rooms.findIndex(r => r.persistentId === roomData.id)
    ...
    const mergeRoomData = (existingRoom, newRoomData) => {
      ...
      if (!mergedRoom.roomNamespace) {
        mergedRoom.roomNamespace = `room-${mergedRoom.inviteHash ? mergedRoom.inviteHash : mergedRoom.id}`.trim();
      }
  ...
  async createRoom(roomData) {
    ...
    const roomNamespace = `room-${roomId}`.trim()
  ...
  async joinRoom(inviteCode, opts = {}) {
    ...
    const roomNamespace = `room-${inviteHash}`.trim()
    ...
    for (const roomInfo of this._rooms) {
      try {
        ...
        if (roomInfo.roomNamespace) {
          ...
          roomNamespace = `room-${roomInfo.id}`.trim();
  ...
  async getRoom(roomId) {
    ...
    let roomInfo = this._rooms.find(r => {
      ...
      const normalizedNamespace = r.roomNamespace ? r.roomNamespace.trim() : null
    ...
    try {
      ...
      // If original namespace was different, also store with that
      if (roomInfo.roomNamespace !== roomNamespace) {
        this._roomInstances.set(roomInfo.roomNamespace, room)
      }
