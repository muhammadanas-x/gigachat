...
class Gigauser extends ReadyResource {
  ...
  async joinRoom(inviteCode) {
    ...
    // First, decode the invite to get some information
    let inviteHash
    try {
      // z32 is used for invite encoding, as seen in GigaRoom
      const decodedInvite = z32.decode(inviteCode)
      inviteHash = crypto.createHash('sha256').update(decodedInvite).digest('hex').slice(0, 16)
    } catch (err) {
      console.error('Error processing invite code:', err)
      inviteHash = crypto.createHash('sha256').update(inviteCode).digest('hex').slice(0, 16)
    }

    // Join the room with deterministic namespace based on invite
    const room = await GigaRoom.joinRoom(
      this.store.namespace(`room-${inviteHash}`), // Deterministic namespace based on invite
      inviteCode,
      {
        owner: this,
