...
class Gigauser extends ReadyResource {
  ...
  async addRoom(roomData) {
    ...
    const mergeRoomData = (existingRoom, newRoomData) => {
      return {
        ...
        namespaceKey: newRoomData.namespaceKey || existingRoom.namespaceKey || null,
  ...
  async _initializeRoomInstances() {
    ...
    for (const roomInfo of this._rooms) {
      try {
        ...
        const roomNamespace = `room-${roomInfo.id}`
        const roomStore = this.store.namespace(roomNamespace)
  ...
  async getRoom(roomId) {
    ...
    console.log('getRoom called', this._roomInstances.entries())
  ...
  static async autoInitialize(store, opts = {}) {
    // Try to retrieve stored context
    const user = new Gigauser(store)
    await user.ready()

    // If no previous context found, create a new user
    if (!user.publicKey) {
      console.warn('No previous user context found. Creating new user.')
      await user.close()
      return this.create(store, opts.seed, opts)
    }

    // Force a full refresh of user data
    await user.refreshUser()
    await user.refreshRooms()
