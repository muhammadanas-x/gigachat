...
class Gigauser extends ReadyResource {
  ...
  async createIdentity(seed) {
    ...
    // Store identity in dedicated collection
  ...
  async _replicate() {
    ...
    this.member = this.pairing.addMember({
      ...
      // In the _replicate method, modify the onadd handler:
      onadd: async (candidate) => {
        try {
          // Get the invite buffer directly from the candidate
          const inviteBuffer = candidate.inviteId
          // Try to find the invite by the invite buffer directly
          let inv = null
          try {
            const stream = this.base.view.find('@gigauser/invite', {})
            for await (const invite of stream) {
              console.log({ inviteBuffer, invite })
