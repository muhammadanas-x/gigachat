...
class GigaRoom extends ReadyResource {
  constructor(corestore, opts = {}) {
    ...
    this.persistentId = opts.persistentId || opts.id || null  // Keep track of original ID
  ...
  async _apply(nodes, view, base) {
    ...
    for (const node of nodes) {
      ...
      if (node.value && typeof node.value === 'object') {
        ...
        if (commandId === 3 || commandId === 4 || commandId === 3 || commandId === 5 || commandId === 3 || commandId === 6) {
          updates.room = true // Room commands
        }
    ...
    if (nodes.length > 0) {
      ...
      if (updates.channels) {
        ...
        console.log("channels synced from peer, emitting channels-updated")
      }
  ...
  async _loadRoomData() {
    try {
      ...
      const originalId = this.persistentId || this.id
      // Load room details
      const roomData = await this.base.view.findOne('@gigaroom/room', {})
      if (roomData) {
        if (originalId && roomData.id !== originalId) {
          console.log(`Room ID mismatch detected - database: ${roomData.id}, persistent: ${originalId}`)
          console.log('Preserving persistent room ID for continuity')
          this.id = originalId
        } else {
          this.id = roomData.id
        }

        // Update persistent ID if it wasn't set
      ...
      if (!this.blobsCoreKey) {
        const k = `room-blobs-${this.id || originalId || this.persistentId || 'default'}`
        const blobsCore = this.store.get({ name: k })
        await blobsCore.ready()
        this.blobsCoreKey = blobsCore.key
        // TODO INIT HYPERBLOBS OF USER IN THIS ROOM 
      }
      ...
      try {
        ...
        this._channels = loadedChannels
      } catch (channelLoadError) {
        console.error('Error loading channels:', channelLoadError)
      }
  ...
  async _replicate() {
    ...
    this._updateInterval = setInterval(async () => {
      try {
        await this.base.update()
        await this._loadRoomData()
        console.log(`Room ${this.id} periodic update: ${this._channels.length} channels loaded`)
      } catch (err) {
        // Silent error - can happen during normal operation
      }
    }, 5000)
  ...
  async createRoom(roomData, creatorPublicKey) {
    ...
    const room = {
      ...
      roomNamespace: roomData.roomNamespace
    }
    ...
    console.log('Ensuring room initiated in db')
    // Reload room data to ensure initialization
    await this._loadRoomData()
  ...
  async createChannel(channelData, creatorPublicKey = null) {
    ...
    if (!this.id) {
      console.error('Room not initialized when creating channel')
    ...
    const roomId = this.persistentId || this.id
    if (!roomId) {
      console.error('Cannot create channel: No room ID available')
      throw new Error('Room not properly initialized')
    }
    ...
    console.log('Attempt to create channel: ', channel)
