...
class GigaRoom extends ReadyResource {
  ...
  async _loadRoomData() {
    try {
      ...
      if (roomData) {
        ...
        // console.log('No room data found in database')
      }
  ...
  async createChannel(channelData, creatorPublicKey = null) {
    if (!creatorPublicKey) creatorPublicKey = this.owner.publicKey
    if (!creatorPublicKey) throw new Error("No public key found")
